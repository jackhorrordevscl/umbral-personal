...
- [x] **0.1** Write a throwaway script (not shipped) that loads one real `GoogleCalendarConnection` refresh token from the DB (dev/staging) and calls `GET /calendars/primary/events?timeMin=...&timeMax=...&singleEvents=true&orderBy=startTime&fields=items(start,end,status,transparency,extendedProperties),nextPageToken` through the existing `GoogleCalendarClient` OAuth client plumbing. **DONE** — script written by the orchestrator (`backend/scripts/pr0-spike-events-list.ts`, never committed), run by the user locally (agent sandbox could not read `backend/.env`), deleted immediately after.
- [x] **0.2** Record the result: 200 with expected fields (proceed with PR 1–4 as designed), or 403 (STOP). **DONE — result: HTTP 200.** `events.list` works under the existing `calendar.events` scope; 0 items returned (empty test calendar), no pagination. Confirms design.md Decision 1 exactly as proposed.
- [x] **0.3** Document the spike result inline. **DONE** — result recorded in `apply-progress.md` ("Resolution" section) rather than as a code comment (the throwaway script was deleted per its own disposability requirement, so there is no shipped file to comment inline).
...
- [x] **5.3 [P]** Update `frontend/src/api/publicScheduling.ts`: add `checkout` field to `BookingConfirmation`, add `getBookingCheckout()` wrapper. **Done (PR 5b)**: also added `groupId` to `BookingConfirmation` (the real response field the checkout endpoint's `:groupId` param needs — `id === groupId` in the first version of a consultation, per `schema.prisma`'s own comment, but the frontend reads it from the response rather than assuming the equality) and exported `CheckoutHint`/`BookingCheckout` types mirroring the backend contract exactly.
- [x] **5.4** Update `PublicBookingPage.tsx` confirmation render: when `checkout.status === 'PENDING'`, poll `getBookingCheckout()` at the proposed 2s interval / ~15s ceiling (design.md still marks this as an open question — implement with these proposed defaults, extract them as named constants so a later PR can tune without touching poll logic) until a `paymentUrl` appears or the budget is exhausted, then render the checkout CTA + "you will leave this page" copy, or fall back to today's "you'll get an email" copy on exhaustion. When `checkout.status === 'NOT_APPLICABLE'` or absent, render only booking success, no CTA. **Done (PR 5b)**: `CHECKOUT_POLL_INTERVAL_MS`/`CHECKOUT_POLL_TIMEOUT_MS` exported as named constants from `PublicBookingPage.tsx`; polling implemented as a plain recursive `setTimeout` inside a `useEffect` scoped to `confirmation.checkout?.status === 'PENDING'` (cancelled on unmount/re-render), not a `react-query` hook — `usePublicScheduling.ts`/`hooks/` was intentionally left untouched, matching design.md's "File Changes" table, which lists only `PublicBookingPage.tsx` and `publicScheduling.ts` for this slice.
- [x] **5.5** Handle `flow_return=1` query param on the confirmation route per spec scenario "Flow return arrival displays confirmation state, not payment status" — **note discrepancy**: this spec scenario references `/book/:therapistId?flow_return=1` as a return destination, but design.md Decision 2 explicitly states the real Flow return already redirects to `/pago-recibido?token=…` (`PaymentReturnPage`), not `/book/:therapistId`, and that no backend/frontend change is needed for the return path. Implement 5.5 as: if `flow_return=1` is ever present on `/book/:therapistId` (e.g., a stale link), render confirmation state without asserting payment status — but do **not** change `PaymentsController`'s redirect target to point here, since design.md's decision supersedes the spec's literal routing detail on this point. Document this resolution in the PR description. **Done (PR 5b)**: `PublicBookingPage.tsx` reads `flow_return` via `useSearchParams()`; when it's `'1'` and no local `confirmation` state exists (i.e., a fresh mount from a stale/old link, not the same-session post-booking render), a generic defensive confirmation screen renders — no availability grid, no checkout polling (no `groupId` is available from a bare query param), no payment-status assertion. `PaymentsController`'s redirect target is untouched, exactly as design.md Decision 2 requires.
...
- [x] **6.1 [P]** Document both flags (`CALENDAR_AVAILABILITY_OVERLAY_ENABLED`, `PUBLIC_BOOKING_CHECKOUT_INLINE_ENABLED`) in whatever env-var reference doc/README section already lists `PUBLIC_SCHEDULING_ENABLED` and similar flags. **Done**: added both rows to the env-var reference table (README.md, next to `PUBLIC_SCHEDULING_ENABLED`) plus a short "Overlay de Google Calendar y checkout en línea" sub-section under "Auto-agenda pública de pacientes" describing both flags and their independence.
- [x] **6.2** Manually verify each flag toggles independently (per proposal.md success criterion "Each flag toggles its feature without affecting the other") — smoke pass, not new automated tests unless a gap is found. **Done, no gap found**: architecturally guaranteed — `AvailabilityService` reads only `CALENDAR_AVAILABILITY_OVERLAY_ENABLED` (`availability.service.ts:222-224`) and `PublicSchedulingService`/`CalendarBusyService` read only their own flag each; neither reads or branches on the other's env var. Smoke evidence: `calendar-busy-overlay.e2e-spec.ts` exercises `CALENDAR_AVAILABILITY_OVERLAY_ENABLED='true'` while `PUBLIC_BOOKING_CHECKOUT_INLINE_ENABLED` stays unset throughout; `public-booking-checkout.e2e-spec.ts` exercises `PUBLIC_BOOKING_CHECKOUT_INLINE_ENABLED='true'` while `CALENDAR_AVAILABILITY_OVERLAY_ENABLED` stays unset throughout; ran both suites together in one Jest invocation (`jest --config ./test/jest-e2e.json calendar-busy-overlay public-booking-checkout`) — 2 suites / 5 tests passed with no interference between the two flag states.
- [x] **6.3** Confirm rollback plan holds: overlay flag off restores byte-identical `computeSlots()` (already covered by PR 2.5's E2E test — re-run it here as the closing gate); checkout flag off restores email-only flow (no new charge surfacing, existing behavior untouched). **Done**: re-ran `calendar-busy-overlay.e2e-spec.ts` standalone — 2/2 passed, byte-identical grid confirmed (flag off is the suite's default state; a manual counter-proof forcing the flag `true` inside the test, documented in apply-progress.md, already proved the assertion is not vacuous). Re-ran `public-booking-checkout.e2e-spec.ts` standalone — 3/3 passed, confirming booking still succeeds end-to-end and the checkout endpoint stays well-formed; the pre-existing email-only path (`ensureCharge()` fire-and-forget, unrelated to either flag) is untouched by this change, so checkout-flag-off is a true no-op on it by construction, not by a new test.
- [x] **6.4** Known-issue note (documentation only, no implementation): a first-time public patient auto-created via public booking has no `defaultSessionAmount`, so no charge is ever generated for them — explicitly out of scope per this task's constraints. Add this as a short note near the checkout code (e.g., a comment on `NOT_APPLICABLE`'s amount-unresolvable branch in PR 5.1) rather than a new doc file, since design.md already raises it as an Open Question. **Confirmed already present** (added in PR 5a): `public-scheduling.service.ts:180-186`, comment block right above `resolveCheckoutHint()`, explicitly labeled `known-issue (design.md Open Questions, tasks.md 6.4)`. No duplicate added.
...
- PR 5b's checkout-polling `useEffect` (5.4) uses a plain recursive `setTimeout`, not a `react-query` `refetchInterval`/custom hook — verified empirically during testing that switching to `vi.useFakeTimers()` *after* a real `setTimeout` was already scheduled (i.e., after the confirmation screen already rendered under real timers) leaves that pending timer running on the real clock, unreachable by `vi.advanceTimersByTimeAsync`; and enabling full fake timers *before* the booking flow instead hangs `@testing-library/dom`'s own `findBy`/`waitFor` internals, which poll via real timers too. The two polling tests (CTA-appears, budget-exhausted) run on real timers with extended `it()`/`waitFor()` timeouts instead — slower (~2s and ~15s respectively) but deterministic and not fighting the test harness.
- PR 5a's checkout GET endpoint (5.2) deliberately reuses the existing `'public-availability'` named throttler bucket instead of registering a third one. `ThrottlerModule` is `@Global()` in this codebase (PR 3's own key learning) — a third named bucket would force `@SkipThrottle` edits across every route in `auth.controller.ts` (~10 routes), `profile.controller.ts` (3 routes), and `email-change.controller.ts` (1 route), all of which already hardcode the exhaustive list of throttler names. Reusing an existing bucket kept the change scoped to `public-scheduling` only, at the cost of the checkout poll sharing its quota with `GET .../availability` (an accepted tradeoff for a read-only endpoint, documented inline in the controller and covered by an explicit "does NOT skip public-availability" test, not just an omission).
